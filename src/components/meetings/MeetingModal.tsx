    ...
    try {
      ...
      if (generatedTasks && generatedTasks.length > 0) {
        ...
        if (tasksError) {
          ...
          console.log('✅ Tareas nuevas guardadas:', savedTasks?.length);

          // 🔔 ENVIAR NOTIFICACIONES
          if (savedTasks && savedTasks.length > 0) {
            console.log('🔔 Enviando notificaciones a responsables...');
            // No esperar a que terminen todas para no bloquear la UI
            Promise.all(savedTasks.map(async (task) => {
              if (task.asignado_a && task.asignado_a !== 'Por asignar' && task.asignado_a !== userEmail) {
                try {
                  await notificationService.createNotification({
                    ...
                    // ID is now auto-generated by the service logic implicitly via DB default, 
                    // but createNotification input type omits it, which matches.
                  });
                  console.log(`__ notification sent to ${task.asignado_a}`);
                } catch (err) {
